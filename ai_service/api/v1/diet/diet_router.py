...
from ai_service.api.v1.diet.diet_schemas import DietCreate, DietUpdate, DietListResponse, DietTrackerListResponse
from ai_service.core.services.diet_service import DietService
...
def get_ai_service(db: AsyncSession = Depends(get_async_db)) -> AuthService:
...
async def create_diet(
    ...
    ai_service: AuthService = Depends(get_ai_service),
):
    """Create a new diet record for the authenticated user"""
    try:
        user = await ai_service.get_user_from_token(authorization)
