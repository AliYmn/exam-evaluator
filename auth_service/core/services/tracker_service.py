...
class TrackerService:
    ...
    async def create_tracker(self, user_id: int, tracker_data: TrackerCreate) -> TrackerResponse:
        ...
        new_tracker = Tracker(user_id=user_id, data=tracker_data.data)
    ...
    async def get_tracker(self, tracker_id: int, user_id: int) -> Optional[TrackerResponse]:
        ...
        result = await self.db.execute(
            select(Tracker).where(Tracker.id == tracker_id, Tracker.user_id == user_id, Tracker.deleted_date is None)
        )
    ...
    async def get_tracker_by_user_id(self, user_id: int) -> Optional[TrackerResponse]:
        ...
        result = await self.db.execute(select(Tracker).where(Tracker.user_id == user_id, Tracker.deleted_date is None))
        tracker = result.scalars().first()
    ...
    async def list_trackers(self, user_id: int, skip: int = 0, limit: int = 100) -> tuple[List[TrackerResponse], int]:
        ...
        # Get total count
        count_query = select(Tracker).where(Tracker.user_id == user_id, Tracker.deleted_date is None)
        count_result = await self.db.execute(count_query)
        total_count = len(count_result.scalars().all())

        # Get paginated records
        result = await self.db.execute(
            select(Tracker).where(Tracker.user_id == user_id, Tracker.deleted_date is None).offset(skip).limit(limit)
        )
